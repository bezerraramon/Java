...
public class Saudacao {
	...
	public String saudar(Relogio r) {
		...
		String txt = "";
		if (r.getHora() > 0 && r.getHora() < 7) {
			txt = "Bom dia!";
		} else if (r.getHora() > 9 && r.getHora() < 12) {
			if (r.getHora() != 12) {
				txt = "Quase na hora de comer!";
			} else if (r.getMinuto() <= 30) {
				txt = "Quase na hora de comer!";
			}
		}else if (r.getHora() == 12 || r.getHora() == 13) {
			if (r.getHora() == 12 && r.getMinuto() >= 30) {
				txt = "Comer!";
			} else if (r.getHora() == 13 && r.getMinuto() <= 30) {
				txt = "Comer!";
			}
		} else {
